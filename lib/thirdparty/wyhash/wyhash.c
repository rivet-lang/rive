...
uint64_t wyhash(const void* key,
                              size_t len,
                              uint64_t seed,
...
// a useful 64bit-64bit mix function to produce deterministic pseudo random
// numbers that can pass BigCrush and PractRand
...
// convert any 64 bit pseudo random numbers to uniform distribution [0,1). It
// can be combined with wyrand, wyhash64 or wyhash.
double wy2u01(uint64_t r) {
  const double _wynorm = 1.0 / (1ull << 52);
  return (r >> 12) * _wynorm;
}

// convert any 64 bit pseudo random numbers to APPROXIMATE Gaussian
// distribution. It can be combined with wyrand, wyhash64 or wyhash.
...
// May not work when WYHASH_32BIT_MUM=1. It can be combined with wyrand,
// wyhash64 or wyhash.
